...
import pymysql, cgi, cgitb, random, string, smtplib
...
for i in res:
    ...
    <tr>
    <form method="post">
     <td>%s</td><input type="hidden" value='%s' name="UID">
     <td>%s</td>
     ...
     <td>%s</td>
     <td>%s</td>
     <td>%s-%s</td><input type="hidden" name="email" value=%s>
     <td><img src="./images/%s" style="height:100px; width:100px;"></td>
    """ % (i[1], i[0], i[2], i[0], i[4], i[5], i[6], i[7], i[2], i[8]))
...
if Submit != None:
    q = """UPDATE business_reg SET password='%s',status="true" WHERE id='%s'""" % (Password,Userid)
    cur.execute(q)
    con.commit()
    print("""
    <script>
    alert("success")
    location.href="new_business.py"
    </script>""")

    fromadd = 'ssnishanthini2002@gmail.com'
    ppassword = 'ytag jiux dlxu wolt'
    toadd = Email
    subject = "password from Endorse"
    body = "hello businessman your password is {}".format(Password)
    msg = """subject:{} \n\n {}""".format(subject, body)

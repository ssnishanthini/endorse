...
if Submit != None:
    ...
    q="""INSERT INTO business_sell(category, brand, title, description, price, photo,sellid)values('%s','%s','%s','%s','%s','%s','%s')"""%(Category, Brand, Title, Description, Price, fn,uid)
    cur.execute(q)
    con.commit()
    print("""
    <script>
    alert('Submitted successfully');
    location.href="business_sell.py?id=%s";
    </script>
    """%(uid))

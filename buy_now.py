...
busid = form.getvalue("bid")
sellid = form.getvalue("sellid")
      ...
      <div class="col-md-2">
        <div class="sidenav">
          ...
          <div class="content">
            ...
            <a href="cart.py?id=%s"><i class="fa-solid fa-list-check"></i> My Orders</a>
...
for j in rec:
    ...
    <div class="form-group>
        ...
        <input type="text" class="form-control" value="%s" name="title" required><br>
        <label for ="description">Description</label>
        <input type="text" class="form-control" value="%s" name="description" required><br>
        <label for ="price">Price</label>
        <input type="text" class="form-control" value="%s" name="price" required>
    </div>
        
        
    """ % (j[6],j[3],j[4],j[5]))

s = """Select * from business_sell where id='%s' """ % busid
cur.execute(s)
rec = cur.fetchall()
for j in rec:
    print("""
...
if Buynow != None:
    q = """INSERT INTO my_orders(uid,image,payment,title,description,price,sellid,uname,uphone,uemail,uaddress,status)values('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','ordered')"""%(uid,Image,Payment,Title,Description,Price,sellid,Uname,Uphone,Uemail,Uaddress)
    cur.execute(q)
    con.commit()
    print("""
        <script>
        alert('Ordered Successfully');
        window.location.href="buy_products.py?id=%s";
        </script>
        """ % (uid))
